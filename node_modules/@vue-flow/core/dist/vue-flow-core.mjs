...
function useActions(id2, state, nodeLookup, edgeLookup) {
  ...
  const findNode = (id22) => {
    ...
    return nodeLookup.value.get(id22);
  };
  const findEdge = (id22) => {
    if (!id22) {
      return;
    }
    return edgeLookup.value.get(id22);
  };
...
function useGetters(state, nodeLookup, edgeLookup) {
  ...
  const getNodesInitialized = computed(() => {
    ...
    for (const node of state.nodes) {
      if (node.initialized && node.handleBounds !== void 0) {
...
class Storage {
  ...
  create(id2, preloadedState) {
    ...
    const actions = useActions(id2, reactiveState, nodeLookup, edgeLookup);
...
const ConnectionLine = defineComponent({
  ...
  setup() {
    ...
    return () => {
      ...
      const toHandle = targetNode && ((_a2 = connectionEndHandle.value) == null ? void 0 : _a2.handleId) && ((connectionMode.value === ConnectionMode.Strict ? (_b = targetNode.handleBounds[handleType === "source" ? "target" : "source"]) == null ? void 0 : _b.find(
        (d) => {
          var _a3;
          return d.id === ((_a3 = connectionEndHandle.value) == null ? void 0 : _a3.handleId);
        }
      ) : (_c = [...targetNode.handleBounds.source || [], ...targetNode.handleBounds.target || []]) == null ? void 0 : _c.find(
        (d) => {
          var _a3;
          return d.id === ((_a3 = connectionEndHandle.value) == null ? void 0 : _a3.handleId);
        }
      )) || ((_d = targetNode.handleBounds[handleType ?? "target"]) == null ? void 0 : _d[0])) || null;
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        (openBlock(true), createElementBlock(Fragment, null, renderList(unref(getEdges), (edge, __, ___, _cached) => {
          const _memo = [edge.id];
          if (_cached && _cached.key === edge.id && isMemoSame(_cached, _memo))
            return _cached;
          const _item = (openBlock(), createElementBlock("svg", {
            key: edge.id,
...
const NodeWrapper = defineComponent({
  ...
  setup(props) {
    ...
    if (node.extent === "parent" || typeof node.extent === "object" && "range" in node.extent && node.extent.range === "parent") {
      until(() => node.initialized).toBe(true).then(clampPosition2);
    } else {
      clampPosition2();
    }
    return () => {
